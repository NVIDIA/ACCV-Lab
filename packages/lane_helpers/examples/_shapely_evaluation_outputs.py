...
def _split_markdown_table_row(line: str) -> list[str]:
    ...
    if not stripped_line.startswith("|") or not stripped_line.endswith("|"):
        raise ValueError(f"Expected Markdown table row, got: {line}")
    cells = [cell.strip() for cell in stripped_line.strip("|").split("|")]
    return cells


# Helper function for loading one metric table written by `_write_table`.
def _read_metric_table(filename: Path) -> tuple[list[int], list[int], np.ndarray]:
    lines = [line for line in filename.read_text(encoding="utf-8").splitlines() if line.strip()]
    if len(lines) < 3:
        raise ValueError(f"Expected at least header, separator, and one data row in {filename}")
...
def _selected_speedup_annotation_cells(
    ...
    # Helper function for finding optional point or distance values in user-provided sweeps.
    ...
    # Helper function for preserving annotation order while avoiding duplicate cells.
    def add_unique_cell(cells: list[tuple[int, int]], cell: tuple[int, int]) -> None:
        if cell not in cells:
            cells.append(cell)

    # Helper function for finding the first faster cell in a fixed point-count row.
    ...
    # Helper function for finding the first faster cell in a fixed distance-count column.
...
def _format_table(
    data: np.ndarray, nums_points: list[int], nums_distances: list[int], *, scientific: bool
) -> str:
    rows = [
...
def _write_table(
    ...
    *,
    ...
    filename.write_text(
        _format_table(data, nums_points, nums_distances, scientific=scientific), encoding="utf-8"
    )
...
# Entry point: write all plots and tables for one evaluated batch size.
def write_batch_results(
    ...
    _write_comparison_outputs(
        ...
        nums_distances,
        metric_data,
        has_shapely_results=not skip_shapely,
        annotate_plots=annotate_plots,
    )


# Entry point: recreate plot images for one batch size from already-written Markdown tables.
def plot_batch_results_from_markdown(
    ...
    markdown_files = sorted(output_dir.glob(f"{prefix}*.md"))
    if not markdown_files:
        raise FileNotFoundError(f"No Markdown result tables found for batch={batch_size} in {output_dir}")
    ...
    if not plotted_files:
        raise FileNotFoundError(
            f"No known Markdown result tables found for batch={batch_size} in {output_dir}"
        )
