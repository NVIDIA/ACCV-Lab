...
at::Tensor polyline_interpolation(at::Tensor points, at::Tensor distances, bool relative_distances) {
    ...
    AT_ASSERTM(points.ndimension() == 3, "points must have shape (batch, num_points, num_dims)");
    AT_ASSERTM(distances.ndimension() == 2, "distances must have shape (batch, num_distances)");
    AT_ASSERTM(points.size(0) == distances.size(0),
               "points and distances must contain the same number of polylines");
    AT_ASSERTM(points.scalar_type() == distances.scalar_type(),
...
at::Tensor polyline_interpolation_var_size_batch(at::Tensor points, at::Tensor distances,
    ...
    AT_ASSERTM(points.ndimension() == 3, "points must have shape (batch, max_num_points, num_dims)");
    AT_ASSERTM(distances.ndimension() == 2, "distances must have shape (batch, max_num_distances)");
    AT_ASSERTM(points.size(0) == distances.size(0),
               "points and distances must contain the same number of polylines");
    AT_ASSERTM(points.scalar_type() == distances.scalar_type(),
               "points and distances must have the same dtype");
    check_same_device(points, distances, "points and distances must be on the same device");
    AT_ASSERTM(sample_sizes_points.scalar_type() == sample_sizes_distances_to_sample.scalar_type(),
    ...
    AT_ASSERTM(sample_sizes_points.ndimension() == 1, "points.sample_sizes must be a 1D tensor");
    AT_ASSERTM(sample_sizes_distances_to_sample.ndimension() == 1,
    ...
    AT_ASSERTM(sample_sizes_points.size(0) == num_samples,
               "points.sample_sizes must contain one count per polyline in points");
    AT_ASSERTM(sample_sizes_distances_to_sample.size(0) == num_samples,
