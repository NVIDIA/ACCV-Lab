...
| [SampleSeparationAccess.py](../samples/SampleSeparationAccess.py) | Demuxer/decoder separation with GOP caching | {py:meth}`~accvlab.on_demand_video_decoder.PyNvGopDecoder.GetGOP`, {py:meth}`~accvlab.on_demand_video_decoder.PyNvGopDecoder.DecodeFromGOPRGB`, `useGOPCache`, `isCacheHit` |
| [SampleSeparationAccessGOPListAPI.py](../samples/SampleSeparationAccessGOPListAPI.py) | Per-video GOP management with caching | {py:meth}`~accvlab.on_demand_video_decoder.PyNvGopDecoder.GetGOPList`, {py:meth}`~accvlab.on_demand_video_decoder.PyNvGopDecoder.DecodeFromGOPListRGB`, `useGOPCache`, `isCacheHit` |
...
| Method | Description |
|--------|-------------|
| `get_cache_info()` | Returns a dictionary with cache statistics |
| `clear_cache()` | Clears all cached GOP data |
...
> recently accessed). For long-running processes with many different videos, use `clear_cache()` to 
